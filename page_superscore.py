...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:8px 0 16px;">
  ...
  <div style="border:2px solid #B91C1C;border-radius:10px;padding:14px 18px;background:#FEF2F2;">
    ...
    <div style="font-size:13px;color:#374151;line-height:1.6;">
      ...
      · 일평균 <b>2~3건</b> 발생<br>
      · <b style="color:#000;">⬛ 텐베거 1000%+ 1건 (0.3%)</b><br>
  ...
  <div style="border:2px solid #F97316;border-radius:10px;padding:14px 18px;background:#FFF7ED;">
    ...
    <div style="font-size:13px;color:#374151;line-height:1.6;">
      ...
      · 일평균 <b>3~5건</b> 발생<br>
      · <b style="color:#000;">⬛ 텐베거 1000%+ 9건 (1.1%)</b><br>
...
""")
    ...
    # === 슈퍼강력매수 vs 추천매수 비교 표 ===
    # 100% 기준 배타적 분포 (합 = 100%) — 한 종목은 한 카테고리에만 속함
    cmp_df = pd.DataFrame([
        {"항목": "매수 건수",            "슈퍼강력매수": "347건",          "추천매수": "808건"},
        # ─── 매도 결과 분포 (배타적, 합계 100%) ───
        {"항목": "⬛ 텐베거 (1000%+)",   "슈퍼강력매수": "1건 (0.3%)",     "추천매수": "9건 (1.1%)"},
        {"항목": "슈퍼위너 (200~1000%)", "슈퍼강력매수": "95건 (27.4%)",   "추천매수": "106건 (13.1%)"},
        {"항목": "100%+ (100~200%)",   "슈퍼강력매수": "67건 (19.3%)",   "추천매수": "108건 (13.4%)"},
        {"항목": "50%+ (50~100%)",     "슈퍼강력매수": "68건 (19.6%)",   "추천매수": "120건 (14.9%)"},
        {"항목": "10%+ (10~50%)",      "슈퍼강력매수": "33건 (9.5%)",    "추천매수": "135건 (16.7%)"},
        {"항목": "소폭익절 (5~10%)",     "슈퍼강력매수": "12건 (3.5%)",    "추천매수": "19건 (2.4%)"},
        {"항목": "보합 (-5~+5%)",       "슈퍼강력매수": "16건 (4.6%)",    "추천매수": "51건 (6.3%)"},
    ...
    st.dataframe(cmp_styler, hide_index=True, use_container_width=True, height=500)
    st.caption(
        "**100% 기준 분포 (배타적)** — 한 종목은 한 카테고리에만 속하며 텐베거~손절까지 합계 100%. "
        "예: 슈퍼강력매수 텐베거 0.3% + 슈퍼위너 27.4% + 100%+ 19.3% + ... + 손절 15.9% = 100%."
    )
...
""")
    ...
    # === 매도 전략 비교: 180일 보유 vs 트레일링/익절 ===
    st.markdown("---")
    st.markdown("### 피크 후 되돌림 — 트레일링 스톱 vs 180일 보유")
    st.markdown(
        "**질문**: 피크 찍고 내려오는 종목, 적정 % 떨어질 때 매도하는 게 나을까? "
        "**답**: 5년 OOS 시뮬레이션 결과 → **180일 그냥 보유가 압도적 최강** (슈퍼위너 한 건이 평균을 끌어올림)."
    )
    exit_path = CACHE / "EXIT_STRATEGY_comparison.csv"
    if exit_path.exists():
        exit_df = pd.read_csv(exit_path)
        # 컬러 스타일 - 최상위 전략 강조
        def _hl_strategy(row):
            if "그냥 보유" in row["전략"]:
                return ["background-color:#FEF2F2;font-weight:700;"] * len(row)
            return [""] * len(row)
        styler = exit_df.style.apply(_hl_strategy, axis=1)
        # 숫자 포맷
        ...
        st.caption(
            "**180일 보유**: 평균 +87%, 슈퍼위너 14.6%, 총 수익 +8,405만 — 최강. "
            "**피크 -10/-15/-20% 트레일링**: 슈퍼위너를 너무 빨리 잘라내서 평균 1~7%로 폭락. "
            "**피크 -30% 트레일링**: 평균 +42%, 그래도 180일 보유의 절반. "
            "**+200% 익절**: 평균 +63%, 슈퍼위너 1/3 이상 잘라냄. "
            "→ **결론: 피크 떨어져도 그냥 들고 가라**. 슈퍼위너는 변동성이 크고, 한 번의 +400% 종목이 평균을 끌어올림."
        )
    else:
        st.info("매도 전략 비교 데이터 없음 (cache/EXIT_STRATEGY_comparison.csv)")

    st.markdown("""
### 왜 180일 보유가 이기나?

1. **슈퍼위너 효과**: 14.6% 종목이 +200% 이상 도달 → 한 종목의 +400% 가 9 종목의 -10% 손실을 커버
2. **변동성 = 슈퍼위너 특징**: 슈퍼위너는 가는 도중 -30%, -40% 자주 출렁임. 트레일링 스톱은 출렁임에서 잘림
3. **세금/거래비용 무시 가능**: 5년 1,155번 매매에 변동 없음
4. **심리적 이점**: 룰이 단순 (사고 잊고 180일 후 팔기) → 매일 차트 안 봐도 됨

### 예외 케이스 (보조 룰)
- 매수일 + 60일까지 한 번도 +10% 못 갔으면 → 약하다 신호 (선택적 조기 매도)
- 다만 OOS 검증으로는 **무조건 180일 보유가 더 나음** 입증됨
""")
