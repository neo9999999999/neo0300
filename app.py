...
def render_strategy_comparison(position_size: int = 100_000):
    ...
    horizons_swing = [("💎 20일 보유", "ret_20d"), ("💎 30일 보유", "ret_30d"),
                       ...
                       ("🥈 240일 보유", "ret_240d"), ("🏆 365일 보유 (TOP1)", "ret_365d")]
    rows_html += (
...
def page_results():
    ...
    # ============================================================
    st.markdown('<div style="height:18px;"></div>', unsafe_allow_html=True)
    # 보유기간 옵션 (실제 컬럼이 있는 것만)
    all_holds = ["365", "240", "180", "120", "90", "60", "30", "20"]
    hold_opts = [h for h in all_holds if f"ret_{h}d" in df_graded.columns]
    if not hold_opts:
        hold_opts = ["120"]  # 최후 fallback
    sort_opts = {
        "newest": "📅 최신순",
        "oldest": "📅 오래된순",
        "return_desc": "🔴 수익률 높은순",
...
## 🚀 NEW: 고급 전략 TOP 3 (분석일: 2026-05-23)

### 🏆 TOP 1 — V/S/A/B 365일 보유 (총 손익 최대)

| 지표 | 값 |
|---|---|
| 6년 누적 손익 | **+9,493만원** (180d 대비 +46.5%) |
| 손익비 | **5.96** |
| 큰수익률 (+50%↑) | **19.5%** (180d 13.9%) |
| 큰손실률 (-30%↓) | 23.1% (180d 16.9%) |
| 필요 자본 | 4,130만원 |
| 연 ROI | +38.3% |
| 거래수 | 1,032건 |

→ **자본 4천만+ 있고 슈퍼위너 (200%+ 종목) 노리고 싶으면 365일 보유**

### 🥈 TOP 2 — V/S/A/B 180일 + 레짐 스케일 (BULL 1.5×/NEUTRAL 1.0/BEAR 0.3) (자본효율 최대)

| 지표 | 값 |
|---|---|
| 6년 누적 손익 | **+7,900만원** (+22%) |
| 손익비 | 5.02 |
| 큰손실/수익 | 16.9%/13.9% (180d 동일, 안정성 유지) |
| 필요 자본 | 3,230만원 |
| **연 ROI** | **+40.8%** (최고) |

→ **시장 환경(BULL/NEUTRAL/BEAR) 자동 감지 → 약세장에는 자본 0.3배로 묶임 ↓**

### 🥉 TOP 3 — V/S/A/B 240일 보유 (절충)

| 지표 | 값 |
|---|---|
| 6년 누적 손익 | **+7,617만원** (+17.6%) |
| 손익비 | **5.49** (개선) |
| 필요 자본 | 2,650만원 |
| 연 ROI | +46.3% |

→ **180일보다 슈퍼위너 더 잡고, 365일보다 자본 덜 묶임**

---

## ⚠️ 익절/손절 vs 그냥 보유 — 충격적 진실

| 전략 | 6년 누적 | 베이스 대비 |
|---|---|---|
| 베이스 (180d 보유) | +6,478만 | — |
| 익절 +30% / 180d | +1,135만 | ❌ -5,343만 |
| 익절 +50% / 180d | +1,551만 | ❌ -4,927만 |
| 트레일링 -20% / 180d | -4만 | ❌ -6,482만 |

→ **익절/손절은 슈퍼위너 +200~500% 종목을 잘라서 모두 손해.
그냥 묶어두는 게 답.**

---

## 🎯 변동성 적응 (HIGH_VOL이 더 좋다!)

| 시장 변동성 | 평균 수익 | 큰손실률 | 권장 비중 |
|---|---|---|---|
| 🔴 HIGH_VOL | **+63.4%** | **6%** | 비중 ↑ |
| 🟡 MEDIUM_VOL | +25% | 15% | 보통 |
| 🟢 LOW_VOL | +5.6% | 22% | 비중 ↓ |

→ **변동성 높을 때가 오히려 안전. "VIX 높으면 빠지자"는 통념 깨졌다.**
...
| 자본 | 추천 전략 | 6년 예상 손익 |
|---|---|---|
| 1,100만 | B급만 (10만/건, 7~25%) | +1,615만 |
| 2,100만 | V/S/A/B 180일 (베이스) | **+6,318만** |
| 2,700만 | V/S/A/B 240일 (TOP 3) | +7,617만 |
| 3,300만 | V/S/A/B 180d + 레짐 (TOP 2) | +7,900만 |
| 4,100만 | V/S/A/B 365일 (TOP 1) | **+9,493만** 🏆 |
        """)
